...
def random_sampling(num_nodes, train_ratio, test_ratio):
    # nodes = np.arange(num_nodes)

    val_ratio = 1 - (train_ratio + test_ratio)
    # train_size = int(num_nodes * train_ratio)
    val_size = int(num_nodes * val_ratio)
    test_size = int(num_nodes * test_ratio) + 1
...
def load_dataset(dataset_name):
    ...
    if dataset_name.lower() == 'computers' or dataset_name.lower() == 'photo':
        # transform = T.Compose([T.ToSparseTensor()])
        dataset = Amazon(dataset_dir, dataset_name.lower())
        data = dataset[0]
        # transform = RandomNodeSplit(num_val=0.1, num_test=0.2)
        # data = transform(data)
        # data.adj_t = data.adj_t.to_symmetric()
        idx_train, idx_val, idx_test = get_train_val_test(data.x.shape[0],
                                                          val_size=0.1,
                                                          test_size=0.2,
                                                          stratify=data.y)
    else:
        dataset_path = f'{dataset_dir}{dataset_name}.pt'
        data = torch.load(dataset_path)
        idx_train, idx_val, idx_test = random_sampling(data.x.shape[0], 0.7, 0.2)

    data.idx_train = torch.tensor(idx_train, dtype=torch.long)
    data.idx_val = torch.tensor(idx_val, dtype=torch.long)
    data.idx_test = torch.tensor(idx_test, dtype=torch.long)

    return data


def get_target_nodes(num_nodes, y):
    num_class = y.max().item() + 1
    target_class = np.random.randint(0, num_class)
