...
def adj_to_tensor(adj):
    ...
    # adj_tensor = torch.sparse_coo_tensor(indices=torch.stack([sparse_row, sparse_col]), values=None,
    #                                      size=torch.Size(adj.shape)).to_symmetric()
    adj_tensor = SparseTensor(row=sparse_col, col=sparse_row, value=None, sparse_sizes=torch.Size(adj.shape),
                              is_sorted=True).to_symmetric()
    adj_tensor = adj_tensor.to_torch_sparse_coo_tensor()
    return adj_tensor
...
def tensor_to_adj(adj_tensor):
    ...
    elif type(adj_tensor) is torch.Tensor and adj_tensor.is_sparse:
        adj_tensor = adj_tensor.cpu()
        indices = adj_tensor._indices()
        row, col = indices[0], indices[1]
        # row = adj_tensor.row
        # col = adj_tensor.col
        data = adj_tensor._values()
        if data is None:
...
def sparse_tensor_to_csr(adj):
    values = adj.values().cpu().numpy()
    row_indices = adj.row().cpu().numpy()
    col_indices = adj.col().cpu().numpy()
    coo_matrix = sp.coo_matrix((values, (row_indices, col_indices)), shape=adj.shape)
