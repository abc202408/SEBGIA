...
from models.attacks import gcn_norm
...
def tdgia_ranking_select(adj, n_inject, n_edge_max, origin_labels, current_pred, target_idx, ratio=0.5, neg=False,
...
def atdgia_ranking_select(adj, n_inject, n_edge_max, origin_labels, current_pred, target_idx, ratio=0.5, neg=False,
                          weight1=0.9, weight2=0.1):
    # ranking with tdgia+ vulnerability score
...
def agia_injection(attacker, model, adj, n_inject, n_edge_max, features, features_attack, target_idx, origin_labels,
    ...
    new_y = new_y[sel_idx]
    print(
        f"Finally injected edges {len(new_x)}, minimum vals {vals[sel_idx].min()}, maximum vals {vals[sel_idx].max()}")
