...
class SEBGIA:
    def __init__(self, edge_index, x, y, idx_train, idx_val, idx_test, k_hop=2, sampling=True, alpha=10,
                 batch_size=4096, lr=0.01, clamp_x=True, nhid=32, weight_decay=5e-4, epochs=500, surrogate_epochs=500,
                 surrogate_model='PrSGC', device='cpu'):
        self.generator = None
        self.surrogate = None
    ...
    def train_surrogate(self):
        ...
        if self.surrogate_model.lower() == 'gcn':
            print('=== Loading GCN surrogate model ===')
            self.surrogate = GCN(nfeat=x.shape[1],
                                 nhid=64,
                                 nclass=y.max().item() + 1,
                                 dropout=0.5, device=self.device).to(self.device)
            self.surrogate.fit(x, edge_index, y, idx_train, idx_val, patience=30)
        elif self.surrogate_model.lower() == 'sgc':
            print('=== Loading SGC surrogate model ===')
            self.surrogate = PrSGC(nfeat=x.shape[1], nclass=y.max().item() + 1, prune=False).to(self.device)
            self.surrogate.fit(x, edge_index, y, idx_train, idx_val, train_iters=self.surrogate_epochs)
        else:
            print('=== Loading PrSGC surrogate model ===')
